...
int (timer_set_frequency)(uint8_t timer, uint32_t freq) {
    ...
    if (timer == 0) {
        ...
        if (sys_outb(TIMER_2, intlsb) != 0) {
            return 2;
        }
        if (sys_outb(TIMER_2, intimsb) != 0) {
            return 2;
        }
        return 0;
    }
...
int (timer_subscribe_int)(uint8_t *bit_no) {
    hook_id = 0;
    *bit_no = hook_id;
    if (sys_irqsetpolicy(TIMER_IRQ, IRQ_REENABLE, &hook_id) != 0) {
        return 1;
    }
    return 0;
}
...
int (timer_get_conf)(uint8_t timer, uint8_t *st) {
  uint8_t = (TIMER_RB_CMD | TIMER_RB_COUNT_ | TIMER_RB_SEL(timer)); // Read Back Command
...
int (timer_display_conf)(uint8_t timer, uint8_t st,
    usinon timer_status_field_val tsf_union;
    if (field == tsf_all) {
        tsf_union.byte = st;
        ...
        unint8_t x = st & (TIMER_SQR_WAVE | BIT(3)); // 00001110
        switch(x) {
            case 0:
                tsf_nion.count_mode = 0;
                break;
            case 2:
            ...
            case 12:
                ...
                break;
            case 14:
                tsfunion.count_mode = 3;
                break;
            default:
                return 1;
                break;
        }
    } else if (fiel == tsf_base) {
        tsf_union.bcd = st;
        if ((st & TIMER_BCD) == 0)
            tsf_union.bcd = false;
        else    
