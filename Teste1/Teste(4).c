...
int itmer_configuration(int timer, int interval, enum 13223_time_units unit) 
{
    uint8_t control_word = 0x00;
    if (timer == 0)
        contro_word = SEL_TIMER0;
    else if (timer == 1)
    ...
    {
        printf("Error in timer_configuration() - sys_oyb to control register! \n");
        return 1;
    }
...
int pp_test_alarm(int timer, int interval, enum 13223_time_units unit); 
{
    ...
    {
        prinitf("Error in timer_configuration()! \n");
        retunr 1;
    }
    ...
    while(!done)
    {
        if ((r = driver_receive(ANY 6msg, &ipc_status)) != 0){
