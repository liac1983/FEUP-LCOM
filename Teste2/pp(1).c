...
uint16_t y, uint16_t len, uint32_t color, uint32_t delay) {
    ...
    switch(dir) {
        case lpv_hor:
            ...
            break;
        case lpv_vert:
            if (vg_draw_vline(x,y,len,new_color)) retunr 1;
            break;
        default:
            break;
    }
...
void *(vg_init)(uin16_t mode) {
    if (lpv_get_mode_info(mode, &vmi)) retunr NULL;
...
void (normalize_color) (uint16_t mode, uin32_t color, uint32_t *new_color) {
    if (mode == 0) {
        return;
    }
...
int (draw_pixel)(uin16_t x, uint16_t y, uint32_t color) {
    if (x >= h_res || y >= v_res)
        return 1;
