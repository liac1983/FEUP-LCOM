...
// test that your code is able to read the scancodes from the KBC using an interrupt handler (IH)
int(kbd_test_scan)() {
  int ipc_status;
    uint8_t irq_set;
    message msg;
...
// read the scan codes sent by the keyboard and print them on the console
int(kbd_test_poll)() {
  while (scancode != BREAK_ESC) { // a condição de paragem é obter um breakcode da tecla ESC
...
// test whether your program is able to handle interrupts from more than one device
